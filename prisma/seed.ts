...
async function createDeterministicWorker() {
    ...
    const workerDocuments = Array.from({length: maxDocuments}, (_, i) => i + 1).map((number) => ({
        worker_id: worker.id,
        document_id: number
    }))
    const workerDocuments2 = Array.from({length: maxDocuments}, (_, i) => i + 1).map((number) => ({
        worker_id: worker2.id,
        document_id: number
    }))
    const workerDocuments3 = Array.from({length: maxDocuments}, (_, i) => i + 1).map((number) => ({
